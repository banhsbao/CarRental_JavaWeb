 ...
 * @author Admin
 */
@WebServlet(name = "SearchByNameHistoryServlet", urlPatterns = {"/SearchByNameHistoryServlet"})
public class SearchByNameHistoryServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (email != null) {
                if (!name.isEmpty()) {
                    ArrayList<HistoryDTO> lisHis = dao.getSearch(email, name);
                    for (HistoryDTO lisHi : lisHis) {
                        lisHi.setRating(dao.getRating(email, lisHi.getIdCar()));
                    }
                    session.setAttribute("HISTORY", lisHis);
                } else {
                    request.setAttribute("NAME_HISTORY", "Please enter somthing to search!");
